...
외래키는 두 개의 테이블을 연결해주는 연결 다리 역할을 합니다. 기본키가 중복된 데이터가 하나의 테이블에 삽입되는 것을 방지하는 역할을 하는 것처럼, 외래키 역시 비슷하게 문제를 방지하는 역할을 수행한다. 외래키는 **새롭게 추가되는 행에서 외래키에 해당하는 값이 외래키가 참조하는 테이블에 존재하는지를 체크한다.
