...
예를 들어 은행 업무에 관련된 데이터베이스 응용 프로그램을 가정하면, 이 프로그램은 계좌이체, 대출, 예금, 출금 등과 같이 은행에서 이루어지는 여러 가지 업무처리에 관련된 세부 프로그램들로 구성된다. 이러한 세부 프로그램들은 사용자 입장에서 보면 하나의 작업 단위가 된다. 그리고 이러한 작업들을 수행하기 위해서는 세부적으로 여러가지의 데이터베이스에 대한 연산(검색, 삽입, 삭제, 수정)들을 필요로 한다.

우리가 데이터베이스에 삽입, 수정, 삭제 등의 작업을 할 때, **여러 개의 작업들을 하나의 작업단위, 트랜잭션으로 묶는다**.
